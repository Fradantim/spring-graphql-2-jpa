...
class BookGraphQLControllerTests {
	...
	void queryBookAllFieldsTest() {
		...
		assertThat(author).hasAllNullFieldsOrPropertiesExcept("id", "name", "country");

		assertThat(author.getCountry()).hasAllNullFieldsOrPropertiesExcept("id", "name");
		...
		for (Person reviewer : book.getReviewers()) {
			assertThat(reviewer).hasAllNullFieldsOrPropertiesExcept("id", "name", "country");
			assertThat(reviewer.getCountry()).hasAllNullFieldsOrPropertiesExcept("id", "name");
		}
	}
	...
	void queryBookOnlyIdsTest() {
		...
		Person author = book.getAuthor();
		assertThat(author).hasAllNullFieldsOrPropertiesExcept("id", "country");
		assertThat(author.getCountry()).hasAllNullFieldsOrPropertiesExcept("id");
		...
		assertThat(book.getReviewers()).hasSize(2);
		for (Person reviewer : book.getReviewers()) {
			assertThat(reviewer).hasAllNullFieldsOrPropertiesExcept("id", "country");
			assertThat(reviewer.getCountry()).hasAllNullFieldsOrPropertiesExcept("id");
		}
	}
	...
	void parallelTest() {
		...
		rootAttributes.forEach(
				ra -> bookAttributes.forEach(ba -> personAttributes.forEach(pa -> quoteAttributes.forEach(qa -> {
					...
					if (!body.isBlank()) {
						queries.add(queryPrefix + body + querySuffix);
					}
				}))));
		...
		queries.parallelStream().forEach(query -> {
			...
			assertThat(response.getBody()).containsKey("data").extracting("data").isInstanceOf(Map.class)
					.hasFieldOrProperty("findBookById").extracting("findBookById").isNotNull().isInstanceOf(Map.class);
		});
	}
